...
#   in-fino-veritas (https://gist.github.com/viktorklang/9fa27179603ed5fe6a1f)
#   fino-time
#   pure
...
repo_info() {
    ...
    if [[ "$1" == "verbose" ]]; then
      ...
      if [[ "$git_is_dirty" != "" ]]; then
         ...
         if [[ "$untracked" != "0" ]]; then
           GST="%{$FG[249]%}, $untracked $(pluralize $untracked 'file') untracked$GST"
         fi
         ...
         if [[ "$git_is_dirty" == "false" ]]; then
            ...
            if [[ "$added" != "0" ]]; then
               GST="%{$FG[249]%}, $added $(pluralize $added 'file') added$GST"
            fi
...
precmd() {
    ...
    print -P "\n╭─[%{$FG[039]%}%*%f] $(user_info)$fg_bold[yellow]%{$terminfo[bold]%} ${PWD/#$HOME/~}%f$(repo_info verbose)%f"
}

user_info() {
    #Uncomment if you only want user & host to be shown when using SSH
    #if [[ -n $SSH_TTY ]]; then
        echo "%{$FG[033]%}%n%f%{$FG[249]%}@%f%{$FG[033]%}%m%f%{$FG[239]%}:%f"
    #fi
}
